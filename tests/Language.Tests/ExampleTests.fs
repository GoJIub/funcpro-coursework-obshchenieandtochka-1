...
let ``map returns squared list`` () =
    ...
    | Ok(VList [ VNumber 1; VNumber 4; VNumber 9; VNumber 16; VNumber 25 ]) ->
        Assert.True(true)
    | result -> Assert.Fail($"Expected squared list, got {result}")
...
let ``filter returns elements greater than 2`` () =
    ...
    | Ok(VList [ VNumber 3; VNumber 4; VNumber 5 ]) ->
        Assert.True(true)
    | result -> Assert.Fail($"Expected (list 3 4 5), got {result}")
...
let ``lazy returns 3`` () =
    ...
    | result -> Assert.Fail($"Expected 3, got {result}")
